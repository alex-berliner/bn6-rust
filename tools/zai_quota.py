#!/usr/bin/env python3
"""The Z.ai GLM coding plan has no balance endpoint (2026-09-16): its limits are per model, per 5-hour window and per
...
def main():
    ...
    try:
        key = json.load(open(os.path.expanduser("~/.pi/agent/auth.json")))["zai"]["key"]
        cfg = tomllib.load(open(os.path.join(os.path.dirname(__file__), "..", "providers.toml"), "rb"))
        run = next((r for r, v in cfg["runs"].items() if any(m.startswith("zai/") for m in v["worker"])), None)
        model = next(m for m in cfg["runs"][run]["worker"] if m.startswith("zai/")).split("/", 1)[1] if run else "glm-5.3-flash"
        body = json.dumps({"model": model, "messages": [{"role": "user", "content": "Reply with the single word: ok"}], "max_tokens": 5}).encode()
        req = urllib.request.Request("https://api.z.ai/api/coding/paas/v4/chat/completions", data=body, method="POST",
                                     headers={"Authorization": "Bearer " + key, "Content-Type": "application/json", "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) pi-agent", "x-zai-session": "probe"})
        try:
        ...
        print("zai: probe error: %r" % (e,)); sys.exit(2)
    recent = 0; log = "/tmp/bn-pacer/zai.log"
    ...
    print("zai: remaining %d (model %s %s); this machine's requests in the log: %d" % (1 if status == 200 else 0, model, "open" if status == 200 else "exhausted (%d)" % status, recent))
