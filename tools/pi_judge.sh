...
Propose the next THREE tickets, in the exact ticket format used in TODO.md, every part present or the ticket is refused by tools/judge_append.py: `### ID. title *(OPEN -- date)*` with a fresh ID, **Why.** with the measured facts, a **Files.** line naming the files the worker may touch, numbered **Do** steps each ending in a measurement, **Rules**, **Acceptance.** (what measured numbers close it) and **Measure and report.**, a **Coordinator:** note, and the docs/SCOPE.md milestone it advances (M1..M11). Keep each ticket under 5000 characters: name the facts and the measurements, do not narrate; every ticket is re-sent to a worker on every turn. For an engine-core (M2) ticket the acceptance is a trace target (field, frames of the scene, first divergent frame) with verify_rows identical as the veto; for a content ticket it is the item's own scene and trace at 0; interaction rules need a scripted-input scenario (one button log driving both sides). Order them by how much trace or row divergence they remove per dollar, against docs/SCOPE.md's milestones (M2 before content; a virus, chip or Navi is a port under the interpreters, never a re-creation). Read only what you need to write them: the open tickets' text from TODO.md, and grep tools/harness.py or TODO_ARCHIVE.md for a specific fact. Reply with the three tickets only." \
