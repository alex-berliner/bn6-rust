...
def main():
    ...
    schema = ('Reply with ONLY the slides, one JSON object per line (JSON Lines, no array, no code fences, no prose), each {"title","codePath","code","text","highlights":[{"code","text"},...]}; newlines inside code are written as \\n. '
              ...
              'write in the text must be explained in the same sentence by what it counts or selects, in plain words -- never by converting '
              'hex to decimal for its own sake (give a decimal only when the size is the point) -- and no number may appear in '
