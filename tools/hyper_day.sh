...
# 10 credits are available, start an all-Hyper run (two worker-hyper slots, coordinator/verifier/recon on
...
# prune verify checkouts and target dirs older than a day
find /tmp/bnwt -maxdepth 1 -name "verify-*" -mtime +1 -exec git worktree remove --force {} \; 2>/dev/null; git worktree prune; find /tmp -maxdepth 1 -name "ct_verify*" -mtime +2 -exec rm -rf {} + 2>/dev/null
pgrep -f 'pi -p --approve --session-dir /tmp/bn-pi' >/dev/null && { echo "a run is active"; exit 0; }
python3 tools/hyper_credits.py --min 10 || { echo "fewer than 10 credits: the tail spends the rest on single-shot sessions"; exec bash tools/hyper_tail.sh; }
