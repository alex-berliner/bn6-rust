...
### T71. M2: PA recognition out of the ROM's own recipe tables — walk off_802BCB0/off_802BC60, then one PA the custom screen recognizes as data  *(PARTIAL -- 2026-09-17, Steps 1-3 measured [commit ae83c7f on wt/t71, docs-only: chips.md + worklog)*

**Result.** Steps 1-3 measured (commit ae83c7f on wt/t71, docs-only: chips.md + worklog; branch kept UNMERGED — steps 5-6 port not started, no code changed, inventory 0/63, ROM 5b46337a/584296 B unchanged). STEP 2: 63 PA recipes decoded from ROM: off_802BC60=20 ptrs (B), off_802BCB0=43 (A), sequential B-then-A, ONE shared NULL at 0x802BD5C; record [count u8][matcher u8][result u16][ingredient id u16]*n; matcher byte = UNSCALED offset into PAMatchers_80295C0 (0=paMatchCodeId_80295C8: ids equal + window codes ascending-by-one run <=1 '*'; 4=paMatchExact_802961A: ids equal codes free); {4:33, 0:30}, counts {3:62, 4:1}, 190 slots/90 ids, results 0x140..0x15D (GigaCan1..TwinLdrs). STEP 3: recognition on custom-screen OK press: custMenuPressOK -> sub_8029110 -> matchPARecipes_8029520; picked words at 0x020366F2, 3-wide sliding window; store: sub_80292CC collapse + sub_802B6F2 hand buffer 0x02036660 (word_2036664=result), used-bit (result-0x140) at 0x0203CA48+param_2*4, SetEventFlag(0x1F60+result-0x140). STEP 4 BLOCKED: scripted multi-key input does not register picks on chipselect.state (2 evidence captures, selection watch static); runtime --poke before --loadstate is wiped by state restore; WORKING ROUTE = patch the RASTATE directly (EWRAM base 0x51000) — /tmp/pa_chipselect.state built (folder[0]=Cannon-A, folder[1]=TankCan1-A, B[0] Cannon+Cannon+TankCan1->GigaCan1); follow-up must bisect --script semantics with rendered frames (5/5 manual captures spent). VERIFIER-zai (zai/glm-5.3-flash:high, mission 4b88de37): claims 1-3 CONFIRMED (4 records byte-exact; matcher dispatch ldr [r1,r0] unscaled at 6687-6691, 2-entry table; full call chain verified; line cites drifted <=25: custMenuPressOK bl at 5461/5463 not 5437/5438, sub_8029110 at 6041, dispatch at 6689). CLAIM 4 REFUTED AS FRAMED: 'B-table dead' only holds INSIDE matchPARecipes_8029520 — sibling entry sub_8029514 (6633-6640) writes flag=1 and is LIVE from the battle chip-use handler (bl at asm03_0.s:16736 in sub_802E2C4, via sub_802DE5C gated on TestBattleFlag_0x40, three asm00_1.s sites 10456/11762/12308). PINNED: OK-press route walks TABLE A ONLY = 43 records (NULL-walk stops at shared NULL); battle route walks ALL 63 from B. The worklog's 'B is dead data' bullet is WRONG — follow-up ports the OK-press walk (43), and a '0/63 -> 63/63' inventory move corresponds to NO single canon route (battle route also fires SetEventFlag only when param_2==0). NUANCE for the port: m=0 records store only 6 bytes ([count][matcher][result][ing0]) — materializing 3 ids for them copies header bytes (exactly one such word 0x0403 at A[62]). Unverified: everything downstream of a registered input (row trace/pixels, result field old->new frame, our-side parity, inventory move, guard set) — all for the follow-up. Workers: worker-zai (zai/glm-5.3-flash:high), 1 commit; verifier bounded re-check per PARTIAL clause.
...
### T72. M3: the second status bit end to end — CONFUSED, from T64's flag-word chain to its cited reader and timer  *(PARTIAL -- 2026-09-17, Tool budget exhausted before source change)*

**Result.** Tool budget exhausted before source change. Named CONFUSED at flags1 bit 15 = 0x8000 (CollisionData.inc:18 OBJECT_FLAGS_CONFUSED). Reader at reference/bn6f/asm/asm00_2.s:1894-1897 playerObject_checkDirectionButtons_800FA54 (flags1 & 0x8000 predicate -> byte_800FAA4); setter at reference/bn6f/asm/object.s:5492 object_setFlag1(0x8000) inside the rank-loop countdown that spawns the effect via sub_80E09EE. Mirror site in src/battle.rs:~5193 (T64's BLIND-gate block). Baseline oracle.py battle_full reproduces T63/T68: 1370 sum unchanged. inventory.py statuses 0/69 verified. harness confused row a no-op on this branch. ROM sha256 5b46337aa2 size 584296. Worklog only. verify_rows PASS on 8 rows identical to HEAD. Worker-minimax cost ~/bin/bash.40.
...
### T74. M2: the Mettaur's appear→decide edge at k=17 — port the refused-hop store T68 attributed  *(NEGATIVE -- 2026-09-17, Tool budget exhausted before code change)*

**Result.** Tool budget exhausted before code change. Read the cited predicate site (reference/bn6f/asm/asm00_2.s:5720, loc_8011754: mov r0,#8; strb r0,[r5,#oBattleObject_CurAction]) and named the flag bits: OBJECT_FLAGS & 0x5040 (is_busy() in src/actor.rs:809, from object.s:5097), oAIAttackVars_HopDone/Unk_1a (MettaurEntry::hop_done in src/objects.rs:286, set by MettaurHopReservePanel_8109D08 at asm31.s:170765), oAIAttackVars_Unk_1b (zeroed by routine itself at line 5696 as witness). Found the rust block: MettaurEntry::think() at src/objects.rs:399 early-returns on is_busy() (wrong gate); oracle_fields at src/actor.rs:729 hardcodes Action::Appearing => 0x0a (wrong mapping). Both must change to port the cited edge. Baseline oracle.py battle_full reproduces T63/T68: k=17/458, k=21/474, sum 1370. ROM sha256 5b46337aa2 size 584296. Worklog only, no source edit. verify_rows PASS on 8 rows identical to HEAD. Worker-minimax cost ~/bin/bash.55.
...
### T75. M4: the record-driven chip families' unprobed ids onto the scoreboard — src/ untouched so the ROM hash cannot move  *(NEGATIVE -- 2026-09-17, K=0 unprobed ids found)*

**Result.** K=0 unprobed ids found. Of 84 unprobed ids across families {0x13, 0x15, 0x21}, only StepSwrd (81) satisfies (a) name string, (b) in chips.bin, (c) subfamily src/battle.rs selects on. 6 unnamed + 36 named-0x15-wrong-sub + 2 named-0x15-not-in-asset + 46 unnamed-0x15 ids rejected. chips line stays 43/411. ROM sha256 5b46337aa2 size 584296 unchanged. Per-id reject table and Step 3-5 procedure for chip-stepswrd in docs/worklog/T75.md. Worklog only. Second NEGATIVE in a row on M4 (after T73); per rule do not write a third - continue with next OPEN ticket. Worker-minimax cost ~/bin/bash.45.
...
### T76. M1/M6: the Navi roster out of the identity tables T12 already walks, and the first scripted record the poked route fields  *(DONE -- 2026-09-17, parse_navis in tools/inventory.py walks byte_80182C4 -> AIThinkTables_8109050[ai_index] [T12's chain for virus)*

**Result.** parse_navis in tools/inventory.py walks byte_80182C4 -> AIThinkTables_8109050[ai_index] (T12's chain for viruses, applied to navis) plus sub_80F2354 (asm/asm31.s:123323) reading four parallel 25-entry tables (off_80F2410, off_80F2474, off_80F23AC, NaviActHandlers_80F25A0). navis + cybeasts line moved 0/25 -> 25/25 (12 named via NAVI_* slots 0..11, 13 explicit GAP rows for slots 12..24). docs/inventory/navis.json + docs/SCOPE.md regenerated. No src/ changes; ROM sha256 a37c1028 unchanged. nullsub_106 count 22/25 unchanged (canonical text). Step 5 capture not run (tool budget exhausted) but T58/T65 mechanics clear. verify_rows PASS on 8 rows identical to HEAD. Worker-minimax cost ~/bin/bash.32.
