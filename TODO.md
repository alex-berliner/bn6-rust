...
### T80. M2: PA recognition end-to-end — finish T71 PARTIAL's scripted-input step and land the recipe walk  *(PARTIAL -- 2026-09-17, Landed PARTIAL on wt/t80 @ e7738ea)*

**Result.** Landed PARTIAL on wt/t80 @ e7738ea. Only step 3 (pa_chipselect_state recipe) landed: tools/states.py State.patches field + _apply_state_patches helper + pa_chipselect_state record (base=CHIPSELECT, patches=((0x5DDC0, 0x01),)); /tmp/pa_chipselect.state sha256 f9935ded0c98849646ef905f43216d808b1ab32e55167a7fabc41ca091e8f79f, byte-identical to T71's on-disk copy (1-byte diff vs chipselect.state at file off 0x5DDC0 0x05->0x01). Steps 4-9 NOT landed (tool budget 80 hard-exhausted): step 4 single-key bisect, step 5 PA_RECIPES const + walk in src/custom.rs, step 6 pa_recog harness row, step 7 F12 pad ladder, step 8 verify_rows on 68-row table, step 9 docs/coverage/pa_recog.md. 13 named rows verified identical to HEAD (cursor 1/1/170, all 12 others 0/0/N). The blocker for the remaining steps remains mgba_capture.c's scripted-input (7ag owns it, off-limits to this ticket). docs/worklog/T80.md carries the full resume path.
