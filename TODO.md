...
### T151. M5 Virus: Mettaur Rank 1 Scenario, Mirroring T87's Lever  *(PARTIAL -- 2026-09-18, state+row landed on branch wt/t151 [c977210+8ff1a02+393070c, sibling-run work independently re-verified this s)*

**Result.** state+row landed on branch wt/t151 (c977210+8ff1a02+393070c, sibling-run work independently re-verified this session): mettaur_rank1 677252/38400/70 neg 642150 NON-BLIND at true residual (rank-1 art unported: kind-0 hardwires METTAUR asset src/battle.rs:2142, src/ excluded by ticket -- T113/T182 convention baseline); mettaur 0/0/70/41734, cursor 1/1/170/186279, wave 0/0/90/3840, window, opening all unchanged (verify_rows PASS from clean checkout); ticket cites corrected by line-by-line read: identity GetVerActorTyAndAIIdx_80182B4 asm00_2.s:20095, table :20117, rank0 idx :20122, rank1 idx :20123 (3-byte rows), ticket's 19965-19974 unrelated; ai4_rank0 canary a phantom (T87 landed no row, trio ai 1/7/17 per :20056-20060); rank lever = T131 OPT-path rec226 quad ids (59:0x02001c2c/30/32), byte_80182C4 runtime poke measured dead; branch base 7e17b94 needs mechanical rebase; art port follow-up ticketed.
...
### T152. M2 battle_full phase gap: close the PARKED-vs-fight divergence on T147's map  *(DONE -- 2026-09-18, trace closed on current main: sequencer 0x0203CA70 canon==rust 0x0 match 40/40, mm/enemy [4,0]/[4,0] k=0/17/39)*

**Result.** trace closed on current main: sequencer 0x0203CA70 canon==rust 0x0 match 40/40, mm/enemy (4,0)/(4,0) k=0/17/39, first divergence none; step 4 required NO new code -- T220's BANNER_RECORD (isBannerBusy_801E754 port) retired the fitted SEQ04_FRAMES arm, original T152 park already on main; src byte-identical, docs-only branch wt/t152 (tip ceffaa3); full 21-row verify_rows PASS, cursor 1/1/170/186279 unmoved, windowclose 63962/buster_charge 2498/gunner 2105613 reproduce main pins (T231 owns windowclose); gunner negative drift 2247584->2284867 noted not investigated
...
### T166. M6 navi spawn gate  *(BLOCKED -- 2026-09-18, ROM walk [0x080a0000..0x08100000] for navi dispatch pattern XX 85 01 returned ZERO hits)*

**Result.** ROM walk (0x080a0000..0x08100000) for navi dispatch pattern XX 85 01 returned ZERO hits. CentralArea1 (0x080b4b78, 14 records) and three event-flagged tables (off_8020170 default / 8020178 EVENT_67F / 8020180 EVENT_680 / 8020188 EVENT_681) carry only ActorType=0 quads (Mettaur/Gunner/ai4 ranks/shells; byte-2 values {0x01, 0x02, 0x04, 0x13, 0x85, 0x1f, 0x37}). oBattleSettings_EnemySetupArrPtr is plain ROM pointer not RAM copy (asm00_1.s:8546-8561). Navi spawn requires ROM-file data patch (extending tools/patch_sterile.py --empty-net-encounter) OR RAM-side fake BattleSettings struct + custom formation + OPT lever (T145 attempted and reported closed — likely T145 setup bug per AUDIT note). src/ change NOT landed — no canon-side state exists to validate the row. Baselines at HEAD 61b4be8: mettaur 0/0/70/41734, cursor 1/1/170/186279, gunner 2105613/38237/130/2284867, navi-gunner 2092176/38237/130/2247584, field-bg1/2/3 5760/5760/5760, buster_charge 2498/188/32/5113 (all byte-identical to T147/T145/T161 pins). T153 plan assembled: KIND_NAVI branch in src/battle.rs:2412 flips (GUNNER, Style::Navi, hp=60) -> (PROTO_MAN, Style::Navi, true_spawn_hp=900), wires byte_80182C4 fill-time + sub_800EC80 spawn-time + byte_81067FC art (0x000B ProtoMan) + byte_8106804 row0 HP (900). ai4_rank0 canary phantom per T153 worklog (T144 rung precedent). Branch wt/T166 aec52f5 KEPT UNMERGED (worklog-only). Next-worker path: T145 setup bug investigation OR ROM-file data patch via patch_sterile.py extension.
...
### T230. Rebase wt/t225 onto post-T216 main: the pad plateau must be re-found, then land the blink  *(DONE -- 2026-09-18, landed d491354: wt/t225 blink rebased onto post-T216 main, conflicts resolved keeping blind_met+emotion)*

**Result.** landed d491354: wt/t225 blink rebased onto post-T216 main, conflicts resolved keeping blind_met+emotion; pad plateau 20/21/22 re-swept (7-point sweep, cursor), pinned 21 (no change vs main); BLIND_POKE_MAILBOX 0x020002f4->0x020002f8 re-nm'd (row's own loud-fail design worked); cite fix :27442/:27478/:27485; verify_rows from clean checkout: emotion_skip 0/0/40 neg 27760, blind_met 0/0/70 neg 60901, mettaur/form_cross/chip-cannon/cursor exact; full isolated table PASS
...
### T232. Mettaur rank-1: name the in-window sheet from OBJ VRAM, then re-port  *(DONE -- 2026-09-18, named-sheet verdict acceptance met [branch wt/t232, kept UNMERGED]: true rank-1 sheet = SpritePointerArr811C3A)*

**Result.** named-sheet verdict acceptance met (branch wt/t232, kept UNMERGED): true rank-1 sheet = SpritePointerArr811C3A4[1] compVirusBattleSpriteMettaur_08241EC4 -- SAME as rank 0, T222's 0x08242E94 was index 2; byte evidence 14-tile blob == canon VRAM tiles 31..44 at f130, OAM rel 0/4/12 -> 31/35/43 == canon in-window pose, container pal[1] == canon OBJ palette row 1; mettaur_v1.bin re-exported (mettaur.bin + pal[1]); row back to 677252/38400/70 neg 642150 not zero; residual anatomy: 96% (652800px) = intro white-fade k0..16 (rust already dark), k20+ silhouette palette never swapped on spawn path (canon 2,2,2 vs rust 41,41,41, row3 keeps pal[0]), col5 stagger 35f-vs-canon-30f (5f late); canaries mettaur/cannon OK, cursor 1129/1111 REGRESSED on wt/t222 substrate (not this diff, T222 port commits never canary-verified) -- fix in the follow-up landing pass
**Why.** T222 (BLOCKED, branch wt/t222 commits e76a54a+a47a69d) ported `compVirusBattleSprite_8242E94` as the rank-1 sheet and the row moved the WRONG way: 677252 -> 728067 (negative 642150 -> 693002). Its canon OAM watch (200 frames, the row's own state) names the defect precisely: the three rank-1 Mettaurs materialize ~35f apart (col4 ~f119, col5 ~f154, col6 ~f184; only two inside the compared window f69..f139), and their in-window visible pose is a STATIC 3-part OAM (8x8, 64x64, 16x16; tiles 31/35/43) in OBJ palette ROW 1 -- while the 8242E94 sheet's anim0 is a 4-5 part cycle at palette 0. Either 8242E94 is the wrong sheet (materialize uses a different one -- T113's materialize art finding in docs/worklog may name it) or the sheet index the rank-1 identity picks differs from the fixture-kind export. wt/t222 is NOT merged; its cherry-picked row+state (from c977210e, conflict-resolved against blind_met) are the substrate -- build on that branch.
**Files.** `tools/states.py` or a probe script (OBJ VRAM dump), `assets/mettaur_v1.bin`, `src/actor.rs`, `src/fixture.rs`, `docs/coverage/mettaur.md`, `docs/worklog/T232.md`. NOT src/battle.rs (T152 lane).
**Do.** 1. Worktree from wt/t222 (bash tools/worktree.sh t232; git checkout -B wt/t232 wt/t222 -- or copy) ; re-run the mettaur_rank1 baseline (expect 728067/38400/70 neg 693002). **measurement.** 2. Dump canon's OBJ VRAM tiles 31/35/43 region (peek 0x06010000+ at the materialize frames f110-f140 on the row's state) and the palette row 1 entries (0x05000200+); match tile contents against candidate sheets in the ROM (compVirus* symbols near 8242E94; rank-1 identity table asm00_2.s:20123 cites its art getter) -- NAME the sheet the in-window pose actually renders from, with byte-level evidence. **measurement.** 3. Fix the export/branch to that sheet (and the materialize-pose handling if the sheet is right but the pose index is not -- cite the OAM setup site). **code.** 4. Re-measure mettaur_rank1; then canaries mettaur 0/0/70/41734, cursor 1/1/170/186279, chip-cannon 0/0/40/9505; full isolated table. **measurement.**
**Rules.** No allowlist change; kind-0 path must stay value-identical (HOP_COOLDOWN = table[0] = 0x1e); no fitted constants -- every byte from ROM export or cited table; if the row cannot reach 0, report with the exact residual + the first divergent frame rather than tuning.
**Acceptance.** mettaur_rank1 0/0/70 non-blind, OR a named-sheet verdict with byte evidence and the residual's first-divergent-frame anatomy in the worklog.
**Measure and report.** baseline · dump match evidence · row · commit · mechanism (true sheet name) · unverified.
**Milestone advanced.** M5.
