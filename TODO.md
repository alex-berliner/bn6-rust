...
### T105. M7 emotion window: pick one reachable emotion state (Full Synchro via Counter Hit, or any pre-battle-set state) and port its face-selection gate  *(PARTIAL -- 2026-09-17, Recon only, from disassembly [worker-zai via prior run, unstamped until now]: emotion byte = eStruct2035280+0x)*
