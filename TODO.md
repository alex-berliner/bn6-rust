...
### T87. M5 — T58 PARTIAL step 5 redo: an AIIndex-4 rank the frame-60 lever can field  *(DONE -- 2026-09-17, Landed 6e493bc: new State battlestart_ai4_rank0 [EVENT_681 flag poke 60:0x02001d58:0x0240 swaps encounter root)*

**Result.** Landed 6e493bc: new State battlestart_ai4_rank0 (EVENT_681 flag poke 60:0x02001d58:0x0240 swaps encounter root to 0x080b50b0 = 12 ungated records; lever 60:0x0200a210:0x37a -> 891 mod 12 = 3 -> rec3 0x080b50e0) fields ai_index-4 rank v0 (NameID 0x0013) at frame 69, slot1 HP 0x5a byte-matches off_8109150[4] elem_hp (0x0810ae4c, stride 6); 40-frame determinism 0 px x2 builds; guard 7 rows identical (cursor 1/1/170 documented tear, not chased). verify_rows PASS 7/7; verifier-zai CONFIRMED build/watch frames, HP table, determinism, mechanism + map-byte wedge negative (iCurrFrame freezes), no-attack-through-330 alignment, additive-only diff; UNCHECKED: census counts 46/19/11 (walker aborted), cite nits asm00_2.s:19965->20000, asm29.s:10488->10493. M5 1/187 ranks fielded; v1/v2 need in-net base states, v3 needs scripted route. Model worker-zai + verifier-zai (glm)
...
### T95. M9 — Audio comparison tool: dump-side CLI that diffs two mgba audio captures sample-exact  *(DONE -- 2026-09-17, Landed DONE on wt/t95 @ 915164a)*

**Result.** Landed DONE on wt/t95 @ 915164a. tools/audio_diff.py CLI with 4 PASS invocations: identical dumps (0/0/-1, exit 0), mutated dump (1 differing sample, first divergence k=2, exit 1), truncated dump (sample-count mismatch), format mismatch (clean error). mgba format cite: tools/mgba_capture.c:30-48 (header), 952-967 (writer: snprintf pname %s/frame.%05d.pcm; fwrite pcm int16_t*2 avail pf). Sample width 16 bits signed, layout interleaved L/R (4 bytes/pair), channels 2, advertised rate 65536 Hz, measured ~96000 Hz. Default flags --format s16le --bits 16 --channels 2 match mgba output sample-exact. docs/handbook/audio.md covers format matrix. 7 guard rows identical to HEAD. Advance M9 tool layer.
...
### T97. M10 — Netbattle recon: handshake + chip-trade routines + UI state field + two-instance scenario design  *(DONE -- 2026-09-17, Landed supplement on wt/t97 @ e828e88 [now d63a3c7 on main])*

**Result.** Landed supplement on wt/t97 @ e828e88 (now d63a3c7 on main). docs/coverage/netbattle_recon.md (209 lines) cross-references the d3f65fc NEGATIVE closure's 16 cites; adds two BattleState-byte candidates (+0x0d BattleState_Unk_0d, +0x11 BattleState_Unk_11 carrying BATTLE_EFFECT_NETWORK_BATTLE=0x04 per constants/enums/battle_constants.inc:5); distinguishes netbattle per-trade-byte routine sub_8039B0A (asm03_1_1.s:2018-2065) from unrelated overworld subsystem_launchChipTrader (asm00_1.s:5694); sketches a full mgba invocation for T97a (mgba_capture.c is off-limits to T97). 7 guard rows identical to HEAD. M10 recon row stays at 1/1 (recon IS the first row).
**Result.** Landed d3f65fc (docs-only): docs/coverage/netbattle.md, 16 cites - handshake libSIO814469C (libs.s:799-807, SIOCNT|=0x80; wireless arm sub_813D648 asm37_0.s:3822-3831 rfu_REQBN_softReset_and_checkID), chip-trade sub_8039B0A (asm03_1_1.s:2018-2065; payload write sub_81469BC libs.s:5713/5715; exchanged pair at 0x201015B/C), UI field BattleSettings.background +0x4 = 0x0200AF64, writer asm03_0.s:14589-14596. NEGATIVE closure per ticket: no netbattle-named symbol in reference/bn6f; UI-arming caller + chip-id semantics of payload named as missing cites; session machine cb_80395A4 on eS200A290 (6-state jump table); BattleSettings is a SINGLETON at 0x0200AF60, not a record table (ticket premise corrected). Scenario frames blocked on mgba link emulation (7ag tooling pass). verify_rows PASS 7/7 identical (cursor 1/1/170 documented tear); no verifier needed. Model worker-zai (glm)
