...
### F12. The chip rows' own residues, family by family *(OPEN -- 2026-09-13, corners row-order pass: poisseed 247/43->115/43 [k9-14 feet only], iceseed/grasseed 132/16->0/0/70 PASS [negs )*
...
**Next pass (human session, 2026-09-13 18:40).** Ticket stays OPEN until every chip row reads 0. Next: poisseed's feet (115 px, k9-14): the feet cover order is reversed for seeds but the same reorder breaks minibomb (bomb OAM5, shadow OAM9/10 below the navi), so the push order has to be per family, the way canon's sheet groups them; then the other non-zero chip rows one family per pass, each with wave/window/opening/chip-cannon and minibomb/iceseed/grasseed as canaries at 0.
