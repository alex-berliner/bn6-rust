...
### T69. M3: finish BLIND — move the render gate to the object's whole OAM commit so the blind row reads 0/0  *(OPEN -- 2026-09-17, PASS 1 LANDED-NOWHERE — branch wt/t69 [tip 8acb12b, code+row 4db0620 on a merge of wt/t64 9f1ce9a] kept UNMERG)*

**Result.** PASS 1 LANDED-NOWHERE — branch wt/t69 (tip 8acb12b, code+row 4db0620 on a merge of wt/t64 9f1ce9a) kept UNMERGED: the row reads blind 1575/466/90 (negative not blind, 14056) against an acceptance of 0/0/90, and no allowed entry covers it. MEASURED (all reproduced by verify_rows from clean checkouts): baseline exactly as published (69987/1940/90 | neg 70487); mailbox still nm 0x020002fc so no harness literal edit needed; ours poked-vs-unpoked over the 90-frame window 38,570 px (worst 664, first nonzero k=1, bbox x164-204 y67-119) pre-fix. HEADLINE, and it corrects T64: the blind row was pinned 73 content-frames wrong — rust capture = canon capture + 73 (mettaur align zero at rust offset 205 pins 140<->213; the blind row pairs canon 40 <-> rust 113), so T64 poked the two sides 73 frames apart and its "our gate hides only ~52%" reading was largely an alignment artifact. SECOND CONTRIBUTOR found: the enemy HP readout (draw_number_in number objects), 315 px/frame in x168-199 y111-120, which T64s gate inside Actor::show never reached; the port lifts the gate to both commit sites (enemy show + the readout loop) via actor::opposing_player_blind(). Canon shape CONFIRMED exactly: canon poked leaves 0 OAM entries in x151-202 y68-120 at captures 41 and 45 where unpoked leaves 6 (readout entry x168 y108 among them) — canon suppresses the whole object. Post-port A/B: ours 72,528 px (max 1,219) vs canon 72,194 over the same window — both numbers reproduce to the pixel, but the ticket demands EQUALITY so the 334 px (0.46%) gap is a miss; ours is the larger side through k~22. Residual 1,575 px fully attributed geometrically: k=0 177 px readout sliver (poke-boundary frame ordering) + k=75-77 466x3 px at x42-69 y66-113 where our hit reaction diverges (canon capture ~115). VERIFIER verdicts (verifier-hyper b0f280f5 + resumed bf77d3e5, hyper/qwen3.8-flash:high, $0.044 nominal, 123,810 tokens): claim 1 CONFIRMED as an effect / REFUTED as an explanation — its own control run (old gate 9f1ce9a, harness-only poke 40->113 + align band 26..39->100..111) gives 29,610/781/90 neg 42,091, so alignment accounts for ~58% of the 68,412 px drop and the gate lift ~42% ("the gain belongs to the gate lift alone" is wrong); claim 2 CONFIRMED; claim 3 CONFIRMED; claim 4 mostly CONFIRMED with one sub-claim REFUTED — the INVIS_HIDE_BIT cite was never stale (asm00_2.s:16840 is the lsr #2 bit-2 test the const names) and was not corrected as the report claims; OBJECT_FLAG_VISIBLE is not what UpdateBattleObjectSprite tests; the gate is read twice per frame, not once (behaviourally identical). Claim 5 CONFIRMED as reported; the pre-port 44,493 figure is the report s own arithmetic (72,528 - 315x89), not an independent measurement. Rules audit clean: 4db0620 touches only docs/worklog/T69.md, src/actor.rs, src/battle.rs, tools/harness.py; no allowlist, no states.py, no statuses.md edit in the code commit, frames=90 and canon_ref=40 unchanged, BLIND_POKES_CANON untouched, negative not blind. Guard rows wave/window/opening/chip-cannon/mettaur/windowclose byte-identical to published; CURSOR MOVED 1/1/170/186279 -> 3/2/186277 and is UNATTRIBUTED — the branch carries T64s src changes too (ROM 582,976 vs ours 583,108 at the ported tip), so the tear may track layout rather than the gate. NEXT PASS must (a) settle the cursor delta by measuring cursor on wt/t64 tip alone, (b) fix the claim-4 comment lines in actor.rs before any re-dispatch, (c) chase the k=0 177 px poke-boundary sliver and the k=75-77 466x3 hit-reaction divergence, (d) close the 334 px A/B gap to exact equality. Worker: worker-hyper 82ccde0b/76c12084, hyper/glm-5.3-flash:high, ~46 min, captures over the ticket guidance (8 custom + ~10 short watch runs) and logged.
...
### T73. M4: close one whole chip family onto the record — grow AS_DATA_FAMILIES from T57's census, delete that arm's `match chip.id`  *(NEGATIVE -- 2026-09-17, Family 0x20 [Recov, 9 ids 154-161+288] is the largest family with every id's behaviour = record byte [RECOV_HP)*

**Result.** Family 0x20 (Recov, 9 ids 154-161+288) is the largest family with every id's behaviour = record byte (RECOV_HP[chip.subfamily]); 8 scoreboard rows all 0/0/40/1731 baseline; conversion arm change ( -> ) regresses cursor guard row from 1/1/170/186279 (HEAD) to 7/6/170/186276, above the ticket's own ≤1 frame/≤1 px gate (T56 code-size mechanism). Reverted src/battle.rs; only worklog commit e9c1fa4 landed. AS_DATA_FAMILIES unchanged (43/411). Per-id byte census + comparison with 0x12 bombs, 0x14 cannon, 0x17 vulcan in docs/worklog/T73.md. Worker-minimax cost ~/bin/bash.66 (3 attempts: T68 failed cold-start twice; T73 completed as NEGATIVE).
