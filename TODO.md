...
### T7q. battle_full k=179 group: add the seq.state gate in Actor::update so the MegaMan executor's per-tick work respects SEQ_20/SEQ_24/SEQ_00/SEQ_04  *(PARTIAL -- 2026-09-15, T7q PARTIAL: seq.state gate added to Actor::update [t1_player_entry early-returns Update::Nothing when seq.sta)*

**Result.** T7q PARTIAL: seq.state gate added to Actor::update (t1_player_entry early-returns Update::Nothing when seq.state in {SEQ_20, SEQ_24, SEQ_00, SEQ_04}). Cites: playerObject_update_80EA484 (asm31.s:107147) + sub_8009158 dispatcher (asm00_1.s:12760) routing to sub_8008452/sub_8008492/sub_800840C/sub_8008064 (asm00_1.s:10441-10514). 4 derived consts added: PLAYER_EXECUTOR_GATED_STATE_{00,04,20,24} tagged // provenance: derived -- off_8008038 entry N. src/battle.rs:3260 now calls t1_player_entry(&mut self.megaman, self.seq.state). Counts DID NOT move (101/86/302): root cause is our self.seq.state stays at SEQ_08 for all k=0..542 in battle_full; canon traverses SEQ_20 (k=31..32) -> SEQ_24 (k=33..132) -> SEQ_00 (k=133..135) -> SEQ_04 (k=136..195). Gate is correct; closing count requires the battle_full trace's seq.state to enter {20,24,00,04} — sequencer-traversal fix out of scope (T7g already named this). Branch wt/t7q merged as f80b72f (src/objects.rs 52 +1, src/battle.rs 2 +1, docs/coverage/battle_full.md 11 +). verify_rows cursor 1/1/170 unchanged. Fitted constants 19 unchanged. Worker model: minimax, 8m52s.
...
### T9i. Gunner as data: fix the harness capture race so the gunner row reads 0/0/N  *(PARTIAL -- 2026-09-15, T9i PARTIAL: gunner race fix landed)*

**Result.** T9i PARTIAL: gunner race fix landed. tools/harness.py:run gains serial flag (default False) + variant_label key; rust+canon serialize in one process bypassing 3-slot semaphore when serial=True; variant_label keeps scratch key unique per ui variant (so ui='both' isolated+integrated runs don't collide). Check.serial: bool = False; gunner Check sets serial=True. Captures clean: rust 170/170, canon 135/135 (no short-count). Gunner row reads 3859001/38400/130 with negative 3838543 not blind — NOT 0/0/130 target. Root cause of remaining residual: alignment mismatch — canon frame 0 is state-loading uniform white, rust origin 8 is battle content; canon_ref=0 assumption is wrong; canon first non-uniform frame is 71. Race fix and alignment are separate issues; ticket scopes race only. Regression set: cursor 1/1/170, mettaur 0/0/70, result 0/0/40, wave 0/0/90, popup 0/0/80 all unchanged. Fitted constants 19 unchanged. tools/harness.py: 56 insertions, 12 deletions. Branch wt/t9i merged as 019a79d. Worker model: minimax, 13m30s.
