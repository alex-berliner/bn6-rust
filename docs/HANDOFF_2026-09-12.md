...
runs reads through a descriptor now, and `tools/verify_rows.py`/`tools/scoreboard.py`'s CLI (the tools that
...
and `tools/verify_rows.py` are deleted — everything they covered reads through a descriptor now
...
- **pi.** Project resources load only after `/trust` is run once in this folder (or
  ...
  tooling objective; close or rewrite it with `/goal` in pi). `the pi workflow section below` and
  `the subagent flows section below` were removed in the cleanup; they are in history at `5ad59b6`.
...
- **Extensions (project-local, `.pi/settings.json` `packages`; installed under `.pi/npm/`, gitignored).**
  ...
  subagent extension** the subagent flows section below gated on approval; nothing custom gets built. Both load: a
