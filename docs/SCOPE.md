...
| M4 | Chips as data: chip object routines ported in coverage order, the custom-screen chip UI, then every standard chip, mega, giga, secret; Program Advances | per chip: scenario, port, trace, pixels | 10 chips pixel-verified AS DATA -- the sword/blade family-0x13 rows (Sword..BambSwrd, Muramasa) dispatch through the record's AttackFamily and the ROM's own tables, T17 -- plus 33 pixel-verified through our own code; StepSwrd (81) rides the same record dispatch without a pixel row of its own (tools/inventory.py AS_DATA_FAMILIES) |
