...
/// flat 0x1e (30) frame wait (CurAction 9, `sub_8109CBC`) the first time
/// `RowCheck` ever runs, then never again (Param4 is left nonzero).
/// asm31.s:171144-171157.
const SPAWN_FRAMES: u16 = 0x1e; // provenance: derived -- sub_810A004, asm31.s:171144-171157
...
/// the module doc for the full trace. Folds in the one-time post-spawn pause
/// (`Param4`'s own gate) as `Spawn`, which the real machine keeps as a
/// separate byte on the object rather than a 6th table entry, but the two
/// are never live at once (`Param4`'s gate only matters before `Unk_00` is
/// ever touched), so one enum covers both without loss.
#[derive(Clone, Copy)]
enum MettaurState {
...
impl Ai {
    pub fn new(style: Style) -> Self {
        Self {
            style,
            pause: ATTACK_PAUSE,
            mettaur: MettaurState::Spawn(SPAWN_FRAMES),
        }
    }
    ...
    fn update_mettaur(&mut self, me: &mut Actor, target: (i32, i32), blocked: u32, rng: &mut Rng) {
        if let MettaurState::Spawn(ticks) = self.mettaur {
            if ticks > 0 {
                self.mettaur = MettaurState::Spawn(ticks - 1);
                return;
            }
            self.mettaur = MettaurState::RowCheck;
        }
