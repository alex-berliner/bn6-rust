...
// per-bit read.
//
// Canon: `blindVisualHandledHere_8016934`'s cross-alliance arm
// (asm00_2.s:16846-16870) clears OBJECT_FLAG_VISIBLE on a cross-side object
// (BattleState+0x0d ^ Alliance != 0, :16849-16852) when
// `battle_findPlayer(alliance ^ 1)` (:16853-16856) carries OBJECT_FLAGS_BLIND.
// The bit is read per object through `object_getFlag` (T18's cited reader,
// asm00_2.s:21655-21661 on disk now; the ticket's "21612 vicinity" predates
// T132's comment insert), which returns `CollisionDataPtr->ObjectFlags1`
// (+0x3c, CollisionData.inc:145). The arm is a RENDER gate only: canon
// re-asserts the header's visible bit 1 every frame (asm00_2.s:16824-16829)
// and clears it again at :16866-16869 while the gate holds, so the object
...
/// the arm's `tst` isolates this one bit (asm00_2.s:16861-16869).
...
/// ObjectFlags1 word (asm00_2.s:21655-21661) and the render arm's `tst`
